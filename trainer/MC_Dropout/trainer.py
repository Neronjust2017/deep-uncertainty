...
class TrainerMc(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            outputs = torch.zeros(data.shape[0], self.model.output_dim, 1).to(self.device)
            ...
            for batch_idx, (data, target) in enumerate(self.valid_data_loader):
                ...
                if samples == 1:
                    ...
                    for i in range(samples):
                        ...
                        loss_i = self._compute_loss(out, target)
                        loss = loss + loss_i
                        outputs[:, :, i] = out
    ...
    def _compute_metric(self, metrics, met, output, target, type="MC_dropout"):
        if self.model.regression_type == 'homo':

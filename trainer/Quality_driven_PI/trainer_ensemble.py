...
class TrainerQdEnsemble(BaseTrainerEnsemble):
    """
    Trainer class
    """
    def __init__(self, models, criterion, metric_ftns, optimizers, config, data_loader,
        ...
        self.train_metrics = MetricTracker(*['loss_' + str(i) for i in range(self.n_ensembles)],
                                           *[m.__name__ + '_' + str(i) for m in self.metric_ftns for i in
                                             range(self.n_ensembles)], writer=self.writer)
        self.valid_metrics = MetricTracker(*['loss_' + str(i) for i in range(self.n_ensembles)],
                                           *[m.__name__ + '_' + str(i) for m in self.metric_ftns for i in
                                             range(self.n_ensembles)], writer=self.writer)
        ...
        for i, (model, optimizer, lr_scheduler) in enumerate(zip(self.models, self.optimizers, self.lr_schedulers)):
            ...
            for batch_idx, (data, target) in enumerate(self.data_loader):
                ...
                for met in self.metric_ftns:
                    self.train_metrics.update(met.__name__ + '_' + str(i), met(output, target, type="QD"))
        ...
        for i, (model, optimizer, lr_scheduler) in enumerate(zip(self.models, self.optimizers, self.lr_schedulers)):
            ...
            with torch.no_grad():
                for batch_idx, (data, target) in enumerate(self.valid_data_loader):
                    ...
                    for met in self.metric_ftns:
                        self.valid_metrics.update(met.__name__ + '_' + str(i), met(output, target, type="QD"))
                    # self.writer.add_image('input', make_grid(data.cpu(), nrow=8, normalize=True))
